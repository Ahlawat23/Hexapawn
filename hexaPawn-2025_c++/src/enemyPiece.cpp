...
std::vector<Square*> EnemyPiece::CalValidMoves(){
    ...
    if(y < GRID_HEIGHT-1){
        ...
        if(x > 0){
            ...
            if(left->currPiece != nullptr && left->currPiece->_type == PieceType::player) validMoves.push_back(left);
        }

        //check on the right square
        if(x < GRID_WIDTH - 1){
            Square* right = Board::instance().grid[x+1][y+1];
            if(right->currPiece != nullptr && right->currPiece->_type == PieceType::player)  validMoves.push_back(right);
        }
    }
