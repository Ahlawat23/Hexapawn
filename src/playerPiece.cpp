...
std::vector<Square*> PlayerPiece::peekValidMoves(){
    ...
    if(y > 0){
        ...
        //check on the forward square if it is idle or not
        Square* forward = Board::instance().grid[x][y-1];
        if(forward->currPiece == nullptr)  validMoves.push_back(forward);
        ...
        if(x > 0){
            Square* left = Board::instance().grid[x-1][y-1];
            if(left->currPiece != nullptr && left->currPiece->_type == PieceType::enemy) validMoves.push_back(left);
        }

        //check on the right square
        if(x < GRID_WIDTH - 1){
            Square* right = Board::instance().grid[x+1][y-1];
            if(right->currPiece != nullptr && right->currPiece->_type == PieceType::enemy) validMoves.push_back(right);
        }
    }
